#!/usr/bin/env bash
# omarchy-trackpad-guard uninstaller (native-DWT design).
#
# Removes the shell plugin, the one-shot helper, the MANAGED libinput quirk
# section (only content between our sentinels; an external equivalent section
# is never touched) and every artifact of the legacy evdev-grab daemon
# (unit, drop-ins, udev rule, ACLs, binary, pid/lock), re-triggering udev so
# coexisting rules (voxtype) re-apply their own ACLs.
#
# Values in ~/.config/hypr/input.lua are never modified. The evtest and acl
# packages are left installed: other software may use them and the new design
# simply no longer requires them.
#
# Test-only overrides (empty/production defaults): OTG_ROOT prefixes the
# /etc paths, OTG_SYSFS_ROOT prefixes /sys. Never set them on a real run.
...
# bin/omarchy-trackpad-guard, install.sh and quirks/*.quirks.in —
...
if (( EUID == 0 )); then
  printf '%s: run this as your normal desktop user, not as root\n' "$prog" >&2
  exit 1
fi
...
# --- legacy cleanup (§8.1) — identical copy in install.sh ---------------------
...
# (identical copy in install.sh). The file is deleted on uninstall only when
# the post-strip remainder is exactly this — user comments count as foreign
# content and always keep the file (§7.2).
...
unmanage_quirk() {
  ...
  if [[ "$status" != "managed" ]]; then
    if [[ "$status" == "external" ]]; then
      note "the quirk section in $QUIRKS_FILE is external (not installed by this plugin); left untouched"
    fi
    return 0
  fi

  # Sentinel sanity: refuse to edit a file whose markers look hand-broken.
  local ns ne first_start first_end
  ns="$(grep -cF -- "$SENTINEL_START" "$QUIRKS_FILE")"
  ne="$(grep -cF -- "$SENTINEL_END" "$QUIRKS_FILE")"
  first_start="$(grep -nF -- "$SENTINEL_START" "$QUIRKS_FILE" | head -n1 | cut -d: -f1)"
  first_end="$(grep -nF -- "$SENTINEL_END" "$QUIRKS_FILE" | head -n1 | cut -d: -f1)"
  if [[ "$ns" != "1" || "$ne" != "1" || "$first_start" -ge "$first_end" ]]; then
    note "warning: malformed omarchy-trackpad-guard sentinels in $QUIRKS_FILE; leaving the file untouched, fix it manually"
    return 0
  fi

  local backup new_content
  backup="$QUIRKS_FILE.bak.trackpad-guard.$(date +%s%N).$$"
  ...
  # Delete the file only when the remainder is byte-for-byte our creation
  # header (user comments or any other content keep it).
  local expected
  expected="$(created_by_us_header)"
  if [[ "$(<"$new_content")" == "$expected" ]]; then
    rm -f -- "$new_content"
    sudo rm -f -- "$QUIRKS_FILE"
    note "removed $QUIRKS_FILE (it only contained the managed section)"
  else
    sudo install -m644 "$new_content" "$QUIRKS_FILE"
    rm -f -- "$new_content"
    note "removed the managed quirk section from $QUIRKS_FILE"
  fi
  note "the quirk change applies at the next login"
}

# --- uninstall --------------------------------------------------------------------

# 1. shell bar-widget plugin (best-effort)
if [[ -d "$PLUGIN_DIR" ]]; then
  ...
  note "removed the shell plugin"
fi

# 2. every legacy-daemon artifact (covers upgrades from ANY earlier version)
cleanup_legacy

# 3. the helper and its ephemeral lock (cleanup_legacy already removed any
# legacy binary at the same path; this also covers the new helper itself)
rm -f -- "$TARGET_BIN" "${XDG_RUNTIME_DIR:-/tmp}/omarchy-trackpad-guard-${UID}.lock"

# 4. managed quirk section (sentinel-owned only)
unmanage_quirk

printf 'Omarchy Trackpad Guard removed (plugin, helper, managed quirk, legacy daemon artifacts).\n'
printf 'The evtest and acl packages were left installed. Values in ~/.config/hypr/input.lua were not modified.\n'
printf 'Note: an externally managed quirk section, if present, was left untouched. Coexisting udev rules (e.g. voxtype'\''s) re-apply their own ACL via the re-trigger — expected and correct.\n'
